...
esp_err_t stm32_ota_begin(stm32_ota_t *stm32_ota) {
  ...
  STM32_ERROR_CHECK(_stm32_write_bytes(stm32_ota, cmd_erase, 2, 1));
...
esp_err_t stm32_ota_write_page(stm32_ota_t *stm32_ota, stm32_loadaddress_t *load_address, const char *ota_data,
  ...
  char cmd_load_address[] = {
      load_address->high_byte, load_address->mid_high_byte,           load_address->mid_low_byte,
      load_address->low_byte,  _stm32_load_address_xor(load_address),
  };
  ...
  esp_err_t err = _stm32_await_rx(stm32_ota, 1);
  if (err != ESP_OK) {
    return err;
  }
